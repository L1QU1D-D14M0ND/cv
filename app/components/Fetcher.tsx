...
function Fetcher() {
    ...
    const [contenido, setContenido] = useState([{ "pagina": "Base", "descripcion": "Plantilla" }]);
        ...
        useEffect(() => {fetch("/contenido.json")
            ...
            <Card.Footer >
                <Skeleton asChild loading={load} >
                    <div className="sectionTinyRow" >
                        { contenido.map( (detalles, index) => <div key={index} > <Heading >{detalles.pagina}</Heading> <Text >{detalles.descripcion}</Text> </div> ) }
                    </div>
                </Skeleton>
            </Card.Footer>
        </Card.Root>
